...
const colourSelector: HTMLSelectElement = document.getElementById(
  'colourSelect'
...
const bodyPartSelector: HTMLSelectElement = document.getElementById(
  'bodyPartSelect'
) as HTMLSelectElement;
...
const statsButton: HTMLElement = document.getElementById(
  'statsBtn'
) as HTMLElement;
statsButton.addEventListener('click', statsBtnHandler);
const statsResultsDiv: HTMLElement = document.getElementById(
  'statsResults'
) as HTMLElement;
function statsBtnHandler() {
  ...
  statsResultsDiv.innerHTML = `${
    Colours[colourSelector.value]
  } spun ${spinCount} times`;
  statsResultsDiv.innerHTML = `${Colours[colourSelector.value]} and ${
    BodyParts[bodyPartSelector.value]
  } spun times`;
  console.log(getColourAmount(Colours[colourSelector.value]));
}
...
function getAmount(colour, bodyPart): number {
  let count = 0;
  for (let c = 0; c < spinHistoryArray.length; c++) {
    if (spinHistoryArray[c].colour == colour) {
      return count++;
    }
  }
}

function getColourAmount(colour): number {
  let count = 0;
  for (let c = 0; c < spinHistoryArray.length; c++) {
    if (spinHistoryArray[c].colour === colour) {
      count++;
